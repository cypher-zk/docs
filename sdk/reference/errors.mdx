...
Thrown by [`readonlyWallet`](/sdk/reference/client#readonlywallet) when a write path tries to sign. Means a mutation hook (`usePlaceBet`, `useClaimPayout`, …) fired against a `CypherClient` whose wallet hasn't finished connecting. Gate the calling UI on your wallet adapter's ready state.

```typescript TypeScript
import { ReadonlyWalletError } from "@cypher-zk/sdk";

class ReadonlyWalletError extends Error {
  readonly code: "READONLY_WALLET";
  readonly name: "ReadonlyWalletError";
  // message defaults to:
  //   "Wallet is read-only. Connect a wallet before sending a transaction."
}
```
