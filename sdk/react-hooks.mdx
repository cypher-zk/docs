---
title: "React Hooks for Cypher Prediction Markets"
...
**Invalidates:** `marketKeys.detail(market)`, `positionKeys.user(user)`
...
**Invalidates:** `positionKeys.user(user)`, `marketKeys.detail(market)`
...
await qc.invalidateQueries({ queryKey: positionKeys.user(userPublicKey) });
```
...
| `positionKeys.user(pubkey)` | `(PublicKey) => QueryKey` | All positions for a user |
